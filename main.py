...
for team in teams:
    ...
    if sum(turbo_drivers) > 0 :
        ...
        turbo_idx =  team_prediction.loc[team_prediction['Name'].isin(turbo_names)]['Race points'].argmax()
        turbo_driver = prediction['Name'].loc[turbo_idx]
        team_points = team_prediction['Race points'].sum() + prediction['Race points'].loc[turbo_idx]
    else:
    ...
    for idx, constructor in constructors.iterrows():
        ...
        team_row = pd.Series({'constructor': constructor['Name'], 'members': members, 'turbo driver': turbo_driver, 'price': price, 'points': points})
